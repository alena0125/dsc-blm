gemma.bslmm.wrapper = function(input, args){
  ...
  train.phenotype.path = paste0(args$result, '_train_phenotype_', input$phenotype.id, '.txt')
  ...
  model_fitting_command = paste("gemma -o", basename(args$result),  "-notsnp")
  ...
  train_predict_command = paste0("gemma -o ", basename(args$result), " -notsnp -epm ./output/",
                                 basename(args$result), ".param.txt -emu ./output/", basename(args$result), ".log.txt -predict 1")
  ...
  gemma.prdt = read.table(paste0('./output/', basename(args$result), '.prdt.txt'))
